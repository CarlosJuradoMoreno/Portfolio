...
export default function Portfolio() {
  ...
  return <main ref={pageRef} id="top" className="overflow-hidden">
    ...
    <section className="py-24 md:py-36"><div className="container-shell"><SectionHead eyebrow="Open source" title="Code in the open." copy="Repository metadata is ready to connect to the GitHub API once your public username is configured."/><div className="grid gap-4">{[["portfolio", "Design-engineered personal portfolio", "TypeScript"]].map((r,i)=><Fade key={r[0]} delay={i*.06}><a href="https://github.com/CarlosJuradoMoreno/Portfolio" target="_blank" rel="noreferrer" className="card card-hover block p-6"><div className="flex items-start justify-between"><Github className="size-5 text-white/40"/><ArrowUpRight className="size-4 text-white/25"/></div><h3 className="mt-8 font-mono text-sm font-medium">{r[0]}</h3><p className="mt-2 text-sm text-muted">{r[1]}</p><div className="mt-6 flex items-center justify-between text-xs text-white/40"><span className="flex items-center gap-2"><i className={`size-2 rounded-full ${i===2?"bg-amber-300":"bg-blue-400"}`}/>{r[2]}</span><span>Updated recently</span></div></a></Fade>)}</div></div></section>
